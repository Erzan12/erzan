...
export default function InvitationsList({ items, type }: ModerationListProps) {
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-6 border-2 border-dashed border-slate-500/10 rounded-[3rem] bg-slate-500/5">
        {type === "pending" ? (
          <>
            ...
            <p className="text-slate-500 text-sm mt-1 text-center">There are no invitations made as of now</p>
          </>
        ) : (
          <>
             <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl shadow-xl mb-4">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">No one responded yet</h3>
            <p className="text-slate-500 text-sm mt-1 text-center">Accepted testimonial invitations will appear here.</p>
          </>
        )}
      </div>
    );
